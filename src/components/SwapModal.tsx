...
import { SwappedWorkout, saveSwappedWorkout, markWorkoutComplete } from "@/lib/storage";
...
export default function SwapModal({ workout, onClose, onSaved }: SwapModalProps) {
  const [type, setType] = useState<SwappedWorkout["type"]>("run");
  ...
  function handleSave() {
    ...
    saveSwappedWorkout(swap);
    markWorkoutComplete({
      workoutId: workout.id,
      completedAt: swap.completedAt,
      notes: `Vervangen door: ${swap.description}`,
    });
    onSaved();
  }
    ...
    >
      ...
      >
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-zinc-900 dark:text-zinc-100">Training vervangen</h3>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">{workout.title}</p>
          </div>
          <button
            onClick={onClose}
            ...
            {swapTypes.map((st) => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-sm font-medium transition-all ${
                  type === st.value
                    ? `${st.color} ring-2 ring-amber-500 ring-offset-1 dark:ring-offset-zinc-900`
